...
class RateLimiter:
    ...
    def wait_if_needed(self, num_requests=1):
        """Wait if necessary to respect rate limits before making num_requests API calls."""
        if num_requests > self.max_requests_per_minute:
            raise ValueError(f"Cannot make {num_requests} requests at once. Maximum is {self.max_requests_per_minute}")
        ...
        if len(self.request_times) + num_requests > self.max_requests_per_minute:
            # Calculate how long to wait until we can make the requests
            # We need to wait until enough old requests have "expired"
            needed_capacity = num_requests
            available_capacity = self.max_requests_per_minute - len(self.request_times)
            
            if available_capacity < needed_capacity:
                # Find when enough requests will expire to make room
                ...
                if len(self.request_times) >= requests_to_expire:
                    ...
                    if wait_time > 0:
                        print(f"Rate limit: {len(self.request_times)}/{self.max_requests_per_minute} requests in last minute.")
                        print(f"Waiting {wait_time:.1f} seconds before making {num_requests} more API calls...")
